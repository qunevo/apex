...
APEX is experimental. Security fixes are developed against the current default branch; older versions do not have a guaranteed backport or long-term support policy. Include the exact revision in reports so maintainers can reproduce the issue. See [releases](https://github.com/qunevo/apex/releases) for any published updates and [publication guidance](docs/publication.md) for release checks.
