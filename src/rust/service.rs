use crate::{compile, engine, model::*, validate};
...
impl Service {
    ...
    fn dispatch(&self, name: &str, args: &Value) -> ToolResult {
        match name {
            ...
            "schedule.create" | "schedule.repair" | "schedule.train" | "schedule.plus"
            | "schedule.improve" | "schedule.evolve" => {
                let key = field(args, "scenario_id")?;
                ...
                let result = if name == "schedule.improve" {
                    ...
                    if let Some(s) = &incumbent {
                        crate::evolution::evolve_from(&scenario.problem, &options, &s.schedule)
                    } else {
                        crate::evolution::evolve(&scenario.problem, &options)
                    }
                } else if name == "schedule.create" {
                    engine::create(&scenario.problem, &options)
                } else if name == "schedule.plus" {
                    crate::search::plus(&scenario.problem, &options)
                } else {
                    engine::train(&scenario.problem, &options)
                };
            ...
            "solver.solve" => Err(error(
                "UNSUPPORTED_BACKEND",
                "No solver is installed. Use schedule.create or schedule.train.",
            )),
