...
use crate::{compile::Compiled, engine, model::*, rules::Customization};
...
pub fn candidate(
    ...
    let transition = engine::transition(p, &m.primary, from, &t.family)?;
    ...
    let penalty = transition.map_or(0.0, |tr| tr.penalty)
        + engine::pattern_penalty(p, &m.primary, sequence, i);
