...
pub trait Customization: Send + Sync {
    ...
    fn evolution_operators(&self) -> Vec<crate::evolution::OperatorDefinition> {
        vec![]
    }
    ...
    fn evolve(
        &self,
        _context: &crate::evolution::OperatorContext<'_>,
    ) -> Result<Option<crate::evolution::Genome>, Diagnostic> {
        Ok(None)
    }
...
pub fn metrics(p: &Problem, s: &Schedule) -> BTreeMap<String, f64> {
    ...
    m.insert("setup_penalty".into(), crate::engine::setup_penalty(p, s));
