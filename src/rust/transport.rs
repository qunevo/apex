...
pub fn tools() -> Value {
    let definitions=tool_names().into_iter().map(|name|{
        let (description,required,properties)=match name{
            ...
            "schedule.improve"=>("Improve a plan using Trainer, Plus with distinct Q policies, and optionally direct schedule GA with adaptive operators (options.improve.evolution_share > 0; default 0 after quality ablations) and one shared time/evaluation budget. Optionally retain a validated schedule_id from the same scenario revision. Preserves the best valid incumbent; no optimality guarantee. Prefer this for user-facing improvement; individual search tools are for diagnostics.",vec!["scenario_id"],json!({"scenario_id":{"type":"string"},"schedule_id":{"type":"string"},"options":options_schema()})),
            "schedule.evolve"=>("Refine operation priorities, modes, routes and conditional choices with a direct GA and adaptive operators. Optional schedule_id retains a validated incumbent from the same scenario revision. Use for explicit direct-search refinements or diagnostic comparisons; extra calls spend an additional budget. No optimality guarantee. Inspect operator evidence and compare with the baseline.",vec!["scenario_id"],json!({"scenario_id":{"type":"string"},"schedule_id":{"type":"string"},"options":options_schema()})),
            "schedule.create"|"schedule.train"|"schedule.repair"|"schedule.plus"=>("Construct and independently validate. Use schedule.create for quick planning and schedule.improve for combined optimization. Train/plus are individual diagnostic methods. Time is checked between worker batches. Inspect search and replay metadata.",vec!["scenario_id"],json!({"scenario_id":{"type":"string"},"options":options_schema()})),
