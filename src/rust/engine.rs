...
fn create_internal(
    ...
    crate::evolution::check_order(p, &options.decision_order).map_err(|e| vec![e])?;
...
pub fn train(p: &Problem, options: &Options) -> Result<Schedule, Vec<Diagnostic>> {
    train_with(p, options, None)
}
fn train_with(
    p: &Problem,
    options: &Options,
    ...
    crate::search::train(p, options, extension)
}
...
pub fn train_customized(
    p: &Problem,
    options: &Options,
    extension: &dyn rules::Customization,
) -> Result<Schedule, Vec<Diagnostic>> {
    train_with(p, options, Some(extension))
}
