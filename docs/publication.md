...
The [benchmark publication](../benchmark/README.md) is distributed as one ZIP beside its reader's guide. It contains the frozen source archive, public scientific instances, completed experiment results, analysis and reproduction scripts, dependency locks and provenance. The original publication manifest and result bytes are preserved inside the ZIP. Local environments, executables, temporary runs and intermediate work remain excluded, as do the separate experiment plan and root benchmark/report helper scripts.
