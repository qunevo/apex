...
| Mandatory dispatch policies | Shared candidate filtering, exact placement probes and deterministic policy replay apply to construction, Trainer, Plus and direct GA. | Supported templates and tested native hooks are bounded; arbitrary legacy policies are not automatically imported. |
...
Current field semantics and operational limits are in the [data model](data-model.md), [operating guide](implementation.md), [material contract](material-dispatch.md), [search contract](search-and-parity.md) and [declarative scheduling contract](architecture/declarative-scheduling.md).
...
- Plus retains a UCT tree; the reviewed v2 explorer selected and committed partial prefixes. The direct Rust GA is a redesigned capability, not a reproduction of v2 random trajectories or annealing.
...
Runnable synthetic coverage is in [core](../tests/core.rs), [parity](../tests/parity.rs), [migration](../tests/migration.rs), [material](../tests/material.rs), [dispatch](../tests/dispatch.rs), [customization](../tests/customization.rs) and [evolution](../tests/evolution.rs). These tests include semantic cases, rejected inputs and corrupted outputs; they establish their tested contracts, not every former customer workflow.
