...
Current portfolio contract for APEX 0.6. The executable input is `apex.v3.4`; `Options.improve` controls the shared search budget. Read the [architecture map](README.md) for module boundaries and [direct evolution](../direct-schedule-evolution.md) for the GA contract.
...
flowchart TD
  ...
  P --> A[Plus: one sequential phase per retained policy]
  T --> E[Shared best valid plan and Pareto archive]
  A --> E
  T -. every evaluation .-> K[Shared dispatch / decoder / validator]
  A -. every rollout .-> K
  A --> G[Optional direct GA: operation orders, modes, routes, conditionals]
  E --> G
  G -. every candidate .-> K
...
1. Trainer receives `trainer_share` of the total evaluation allowance (floor, at least one) and approximately that share of the remaining time; the default is 40%. Its initial population includes the requested constructive strategy, so a separate duplicate baseline construction is unnecessary. An optional incumbent is retained separately.
...
4. The optional direct GA receives its remaining global allowance and a bounded pool of validated schedules from earlier phases. Its operators and discounted-UCB selection are described in the [direct-evolution contract](../direct-schedule-evolution.md). The default reserve is zero; a positive `evolution_share` enables this phase. Historical fixed-share regressions are recorded in the [migration audit](../migration-audit.md).
...
There is no adaptive budget learning, resumed cross-request search, alternating Trainer/Plus feedback cycle or automatic translation of a successful Plus sequence back into Q weights.

## Implementation responsibilities

| Module | Responsibility |
| --- | --- |
| [improve.rs](../../src/rust/improve.rs) | `improve_from` checks options and incumbent, allocates phase budgets, retains seeds, and merges progress, archive and best result. |
| [search.rs](../../src/rust/search.rs) | Trainer evolves policy configurations; Plus explores prefixes and rolls out using a retained policy. Both report evaluated candidates to the portfolio. |
| [evolution.rs](../../src/rust/evolution.rs) | Evolves priority chromosomes and mode/route/conditional genes from validated schedules; selects and credits built-in or registered native operators. |
| [engine.rs](../../src/rust/engine.rs) | `evaluate` runs common lowering, dispatch, decoding, exact metrics and validation. A search strategy cannot opt out of hard rules. |
| [service.rs](../../src/rust/service.rs) | Verifies incumbent scenario identity and revision before invoking the portfolio; saves the accepted result with its problem snapshot. |
...
Use [combined-improvement tests](../../tests/improve.rs) for incumbent compatibility, shared budgets, fallback and reproducibility; [search tests](../../tests/search.rs) for Trainer/Plus semantics; and [evolution tests](../../tests/evolution.rs) for commitment preservation, genetic proposals and operator evidence. Add rejected-input and corrupted-result cases when changing these contracts.

Quality comparisons require identical total nominal budgets, reported overruns and actual schedule objectives; simply spending both standalone budgets would not establish an advantage. A larger portfolio or a third phase is not guaranteed to improve quality. The [migration audit](../migration-audit.md) retains historical comparisons; [search semantics](../search-and-parity.md) and [declarative scheduling](declarative-scheduling.md) define current behavior.
