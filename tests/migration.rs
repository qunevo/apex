use apex::{compile, demo, engine, improve, material, model::*, search, validate::validate};
use serde_json::json;
...
fn conditional_alternatives_improve_complete_schedules_in_both_searches_and_replay() {
    ...
    for s in [
        search::train(&p, &o, None).unwrap(),
        search::plus(&p, &o).unwrap(),
...
fn conditional_commitments_conflicts_corruption_and_parallel_search() {
    ...
    parallel.trainer.workers = 2;
    let a = search::train(&p, &o, None).unwrap();
    let b = search::train(&p, &parallel, None).unwrap();
...
fn urgency_propagates_across_jobs_and_merges_without_changing_business_objectives() {
    ...
    let s = engine::create(&p, &options()).unwrap();
...
fn pegging_preserves_routes_and_reallocates_actual_producers_without_double_supply() {
    ...
    for (route, producer, end) in [("slow", "T000000", 70.0), ("fast", "T000001", 30.0)] {
        ...
        for s in [
            engine::create(&q, &o).unwrap(),
            search::train(&q, &o, None).unwrap(),
            search::plus(&q, &o).unwrap(),
        ] {
            ...
            let replay = engine::create(&q, s.replay.as_ref().unwrap()).unwrap();
...
fn material_shortage_in_one_route_does_not_discard_a_feasible_alternative() {
    ...
    assert!(validate(&q, &engine::create(&q, &good).unwrap()).valid);
    let s = search::plus(&q, &options()).unwrap();
...
fn new_kpis_have_explicit_group_units_and_are_independently_checked() {
    ...
    assert!(validate(&p, &search::train(&p, &options(), None).unwrap()).valid);
}

#[test]
fn productive_utilization_excludes_pauses_and_counts_conditional_cost_separately() {
    let p = fixture();
    let s = engine::create(&p, &prefix(&p)).unwrap();
...
fn mode_specific_conditionals_and_depth_boundary_remain_searchable() {
    ...
    let mut o = prefix(&p);
    o.plus.depth = 1;
    assert_eq!(search::plus(&p, &o).unwrap().metrics["makespan"], 100.0);
}
...
fn restart_and_transition_alternatives_are_pinned_and_revalidated() {
    ...
    let s = search::plus(&p, &prefix(&p)).unwrap();
    assert!(validate(&p, &s).valid);
    assert!(
        s.assignments[0]
            .activities
    ...
    assert!(engine::create(&p, &wrong).is_err());
}
...
fn native_compile_can_supply_searchable_conditional_alternatives() {
    ...
    let o = prefix(&p);
    for s in [
        search::train(&p, &o, Some(&AddPreparation)).unwrap(),
        search::plus_customized(&p, &o, Some(&AddPreparation)).unwrap(),
    ] {
        assert_eq!(s.metrics["makespan"], 100.0);
        let (lowered, replay) =
            engine::create_customized(&p, s.replay.as_ref().unwrap(), &AddPreparation).unwrap();
...
fn fractional_objectives_and_native_metrics_survive_json_persistence() {
    ...
    for metric in ["dummy_priority_completion", "task_on_time_delivery"] {
        ...
        let s = search::train(&p, &options(), None).unwrap();
