use apex::{compile::compile, demo, engine, model::*, validate};
...
#[test]
fn separate_sequence_penalties_drive_training_and_are_revalidated() {
    let mut p = problem(2);
    ...
    let fast = engine::create(&p, &Options::default()).unwrap();
    ...
    .unwrap();
    assert_eq!(fast.metrics["setup_penalty"], 200.0);
    assert_eq!(trained.metrics["setup_penalty"], 0.0);
    assert_eq!(fast.metrics["makespan"], trained.metrics["makespan"]);
    let mut bad = trained;
...
fn frozen_sequence_does_not_become_a_product_release_dependency() {
    ...
    for s in [
        engine::create(&p, &options).unwrap(),
        engine::train(&p, &options).unwrap(),
        apex::search::plus(&p, &options).unwrap(),
    ] {
        assert_eq!(s.assignments[0].ready, 110);
        assert_eq!(s.assignments[1].start, 10);
        assert!(validate::validate(&p, &s).valid);
