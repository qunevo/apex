use apex::{demo, engine, model::*, policy, rules::Customization, search, validate};
...
fn candidate_beyond_window_is_found_and_prefix_cannot_bypass_policy() {
    ...
    assert_eq!(
        engine::create(&p, &options).unwrap_err()[0].code,
        "DISPATCH_PREFIX"
    );
...
fn singleton_fallback_is_explicit_and_fixed_start_does_not_silently_override_policy() {
    ...
    assert!(engine::create(&p, &o()).is_ok());
    set(&mut p, "on_no_match", json!("fail"));
    assert_eq!(
        engine::create(&p, &o()).unwrap_err()[0].code,
        "DISPATCH_CONFLICT"
    );
...
fn every_strategy_and_both_searches_share_enforcement_and_parallel_replay() {
    ...
    let mut options = o();
    options.trainer.workers = 2;
    options.plus.workers = 2;
    let a = search::train(&p, &options, None).unwrap();
    assert!(a.search.as_ref().unwrap().generations > 0);
    assert!(a.search.as_ref().unwrap().mutations > 0);
    let mut serial = options.clone();
    serial.trainer.workers = 1;
    let b = search::train(&p, &serial, None).unwrap();
    assert_eq!(a.score, b.score);
    assert_eq!(seq(&a), seq(&b));
    let s = search::plus(&p, &options).unwrap();
    assert!(validate::validate(&p, &s).valid);
    assert_ne!(seq(&s)[0], "T0");
}
#[test]
...
fn declarative_constraints_and_completion_objective_equal_explicit_model() {
    ...
    let a = engine::create(&p, &o()).unwrap();
    let b = engine::create(&lowered, &o()).unwrap();
    ...
    let s = search::train(&p, &o(), None).unwrap();
    assert!(validate::validate(&p, &s).valid);
...
fn native_filter_gets_full_pool_and_cannot_be_bypassed_by_forced_decisions() {
    let p = problem(3);
    let (_, s) = engine::create_customized(&p, &o(), &RejectFirst).unwrap();
    assert_ne!(seq(&s)[0], "T0");
    ...
    assert!(engine::create_customized(&p, &options, &RejectFirst).is_err());
    assert!(engine::train_customized(&p, &o(), &RejectFirst).is_ok());
}
#[test]
fn exact_rules_reject_extensions_without_a_prefix_decoration_contract() {
    let mut p = problem(2);
    campaign(&mut p, "work", 20.0);
    let errors = engine::create_customized(&p, &o(), &RejectFirst).unwrap_err();
...
fn direct_calendar_probe_matches_full_decoder_with_breaks_rates_locks_and_modes() {
    for seed in 0..12 {
        ...
        let a = engine::create(&p, &o()).unwrap();
        let (_, b) = engine::create_customized(&p, &o(), &PrefixSafe).unwrap();
        assert_eq!(seq(&a), seq(&b));
        assert_eq!(
            serde_json::to_value(&a.assignments).unwrap(),
            serde_json::to_value(&b.assignments).unwrap()
...
#[test]
fn custom_filter_is_enforced_in_plus_and_corrupted_explanations_are_rejected() {
    let p = problem(3);
    let s = search::plus_customized(&p, &o(), Some(&RejectFirst)).unwrap();
    assert_ne!(seq(&s)[0], "T0");
...
#[test]
fn plus_can_expand_a_valid_prefix_even_when_its_greedy_suffix_fails() {
    let mut p = problem(3);
    ...
    assert!(engine::create(&p, &options).is_err());
    let s = search::plus(&p, &options).unwrap();
    assert_eq!(seq(&s)[0], "T1");
    assert!(validate::validate(&p, &s).valid);
}
...
fn policy_counts_observed_and_remaining_running_work() {
    ...
    let original = engine::create(&p, &Options::default()).unwrap();
...
fn native_hook_cannot_reject_an_unknown_pair() {
    let error = engine::create_customized(&problem(2), &o(), &InvalidFilter).unwrap_err();
...
fn native_hook_can_request_exact_preparation_facts() {
    ...
    let (_, s) = engine::create_customized(&p, &o(), &TimedFilter).unwrap();
