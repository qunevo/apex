//! XE: direct schedule evolution. Operators propose priorities/choices, never relax feasibility.
use crate::{
    model::*,
    rules::Customization,
    search::{self, Random},
...
pub struct Config {
    ...
    /// None preserves the caller's existing XH population setting.
        ...
        let modes = p
            ...
            .filter_map(|t| {
                let choices: Vec<_> = t
                    .modes
                    .iter()
                    .filter(|m| a.modes.get(&t.id) != Some(&m.id) && xg::allowed(p, t, m))
                    .map(|m| m.id.clone())
                    .collect();
...
fn select(population: &mut Vec<(Genome, Schedule)>, o: &Options) {
    ...
    let order = if o.xh.selection == "pareto" {
        search::pareto_order(
            &population
                .iter()
    ...
    *population = order
        .into_iter()
        .take(o.xe.population_size.unwrap_or(o.xh.population_size))
...
pub(crate) fn evolve_observed(
    ...
    let native = extension.map(|e| e.xe_operators()).unwrap_or_default();
    let mut unique = HashSet::new();
    if native.len() > 32
        || native
            .iter()
    ...
    for (_, s) in &population {
        search::archive(&mut report, s, o.xh.archive_limit);
    }
    ...
    loop {
        if report.evaluations >= max
            ...
            || report.generations > 0 && o.xh.generations.is_some_and(|g| report.generations >= g)
        {
            break;
        }
        let count =
            o.xe.population_size
                .unwrap_or(o.xh.population_size)
        ...
        for _ in 0..count.saturating_mul(32) {
            ...
            let ai = parent_index(population.len(), &o.xe, &mut rng);
            let mut bi = parent_index(population.len(), &o.xe, &mut rng);
            ...
            let index = arm(&report.operators, &pending, &eligible, &o.xe, &mut rng);
            ...
            if candidate.is_ok() && (unchanged || key.as_ref().is_some_and(|k| seen.contains(k))) {
                ...
                credit(&mut report.operators, index, &o.xe, 0.0);
                continue;
            }
            if let Some(key) = key {
                seen.insert(key);
        ...
        for chunk in proposals.chunks(o.xh.workers) {
            ...
            let jobs: Vec<_> = chunk
                ...
                .collect();
            let mut results = search::parallel(p, &jobs, o.xh.workers, extension).into_iter();
            for (candidate, index, parent, unchanged) in chunk {
                    ...
                    Ok(mut s) => {
                        if let Some(index) = index {
                            ...
                            if let Some((score, objectives)) = parent {
                                let better = if o.xh.selection == "pareto" {
                ...
                if let Some(index) = index {
                    credit(&mut report.operators, *index, &o.xe, reward);
                }
            }
        }
