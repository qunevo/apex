...
use crate::{
    compile::Compiled, language::Selector, model::*, queues::Candidate, rules::Customization,
    xg::Decisions,
};
...
fn probe(
    ...
    let s = if let Some(extension) = extension {
        ...
        crate::xg::decode_prefix(
            &crate::compile::compile(&decorated).map_err(|e| e[0].clone())?,
            &next,
        )?
    } else {
        crate::xg::decode_prefix(c, &next)?
    };
...
pub fn verify(
    ...
    let expected = if let Some(extension) = extension {
        ...
        crate::xg::decode(&crate::compile::compile(&decorated)?, &d)
    } else {
        crate::xg::decode(&c, &d)
    }
    .map_err(|e| vec![e])?;
