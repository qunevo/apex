//! Shared-budget XH -> XT -> optional XE portfolio (XHT or XHTE).
...
pub fn improve_from(
    ...
    crate::xe::check_config(o)?;
    if !(0.0..1.0).contains(&o.improve.xh_share)
        || o.improve.xh_share == 0.0
        || !o.improve.xe_share.is_finite()
        || o.improve.xe_share < 0.0
        || o.improve.xh_share + o.improve.xe_share >= 1.0
        ...
        || (max == usize::MAX && o.budget_ms == 0)
    {
        return Err(vec![Diagnostic::new(
            "IMPROVE_OPTIONS",
            "improve",
            "Require xh_share > 0, xe_share >= 0, their sum < 1, 1..8 policies, and an evaluation or time limit",
        )]);
    }
    let owned = p
        .customization
    ...
    let mut state = Portfolio {
        start,
        report: SearchReport {
            algorithm: if o.improve.xe_share > 0.0 {
                "XHTE"
            } else {
                "XHT"
            }
            .into(),
            workers: o.xh.workers.max(o.xt.workers),
            ..Default::default()
        },
        best: None,
        seeds: vec![],
        ...
        errors: vec![],
        schedules: vec![],
        population_size: o.xh.population_size,
    };
    ...
    if let Some(s) = incumbent {
        ...
        state.accept(s, o.xh.archive_limit);
    ...
    if !exhausted(&state) {
        let mut xh_options = base.clone();
        xh_options.xh.max_evaluations = Some(if max == usize::MAX {
            0
        } else {
            ((max as f64 * o.improve.xh_share).floor() as usize)
                .max(1)
                .min(max)
        });
        if o.budget_ms > 0 {
            ...
            xh_options.budget_ms = ((remaining as f64 * o.improve.xh_share).floor() as u64).max(1);
        }
        let phase_start = Instant::now();
        let before = state.report.evaluations;
        let failed = state.report.failed_evaluations;
        let result =
            crate::xh::search_observed(p, &xh_options, extension, &mut |genome, result| {
                state.observe(result, o.xh.archive_limit);
                if let Ok(s) = result {
    ...
    let mut seeds: Vec<_> = pool
        ...
        .collect();
    // A failed greedy population is not proof of infeasibility: XT can backtrack.
    ...
    let reserve = if max == usize::MAX {
        ...
        (max as f64 * o.improve.xe_share).floor() as usize
    };
    let reserve_ms = (o.budget_ms as f64 * o.improve.xe_share).floor() as u64;
    for (index, (mut options, initial)) in seeds.into_iter().enumerate() {
        ...
        options.xh.max_evaluations = Some(if max == usize::MAX {
            0
        } else {
        ...
        let phase_start = Instant::now();
        let result =
            crate::xt::search_observed(p, &options, extension, initial, &mut |_, result| {
                state.observe(result, o.xh.archive_limit);
            });
        let report = result.as_ref().ok().and_then(|s| s.search.as_ref());
        if let Some(r) = report {
        ...
        state.report.phases.push(SearchPhase {
            name: format!("{}:{}", crate::xt::ID, index + 1),
            evaluations: state.report.evaluations - before,
            failed_evaluations: state.report.failed_evaluations - failed,
            elapsed_ms: phase_start.elapsed().as_secs_f64() * 1000.0,
            stop_reason: report.map_or("no_valid_result", |r| &r.stop_reason).into(),
    ...
    if o.improve.xe_share > 0.0 && !exhausted(&state) {
        ...
        let result =
            crate::xe::evolve_observed(p, &options, extension, &seeds, &mut |_, result| {
                state.observe(result, o.xh.archive_limit)
            });
        let report = result.as_ref().ok().and_then(|s| s.search.as_ref());
        if let Some(r) = report {
