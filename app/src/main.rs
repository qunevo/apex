use apex::{demo, model::*, service::Service, transport, validate, xg};
use serde_json::json;
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match command {
        ...
        "plan" | "hypersearch" | "treesearch" | "improve" | "evolve" => {
            ...
            if let Some(value) = flag("--workers") {
                opt.xh.workers = value.parse()?;
                opt.xt.workers = value.parse()?;
            }
            if let Some(value) = flag("--generations") {
                opt.xh.generations = Some(value.parse()?);
            }
            let result = match command {
                "hypersearch" => apex::xh::search(&p, &opt),
                "treesearch" => apex::xt::search(&p, &opt),
                ...
                "evolve" => {
                    if let Some(path) = flag("--incumbent") {
                        ...
                        apex::xe::evolve_from(&p, &opt, &incumbent)
                    } else {
                        apex::xe::evolve(&p, &opt)
                    }
                }
                _ => xg::create(&p, &opt),
            };
            ...
            for size in sizes.split(',') {
                ...
                for run in 0..repeats {
                    for xh in [false, true] {
                        ...
                        let s = if xh {
                            apex::xh::search(&p, &opt)
                        } else {
                            xg::create(&p, &opt)
                        }
                        .map_err(|e| serde_json::to_string(&e).unwrap())?;
                        rows.push(json!({"tasks":count,"run":run,"method":if xh{apex::xh::ID}else{apex::xg::ID},"elapsed_ms":s.elapsed_ms,"evaluations":s.evaluations,"score":s.score,"metrics":s.metrics,"valid":true}));
                    }
                }
            }
            let result = json!({"xg":env!("CARGO_PKG_VERSION"),"os":env::consts::OS,"arch":env::consts::ARCH,"profile":if cfg!(debug_assertions){"debug"}else{"release"},"dataset":"deterministic synthetic independent tasks, two modes, four machines, no breaks or conditionals","includes":"compile, construct, decode, independent validation","rows":rows});
