...
APEX uses four component names consistently in Rust modules, options and search reports: XG constructs schedules, XH searches dispatch policies, XT searches construction prefixes, and XE evolves schedules directly. Use `schedule.create` or `schedule.improve` in planner conversations; [combined improvement](architecture/combined-improvement.md) shares one budget across XH, XT and optional XE. Standalone search tools remain available for explicit diagnostics. The [data-model reference](data-model.md) defines material-mode choices, conditional alternatives, chain urgency and KPI units.

## Component names and API migration

| Component | Rust module | API tool | CLI command | Options |
| --- | --- | --- | --- | --- |
| XG | `xg` | `schedule.create` | `plan` | Shared construction fields in `Options` |
| XH | `xh` | `schedule.hypersearch` | `hypersearch` | `xh` |
| XT | `xt` | `schedule.treesearch` | `treesearch` | `xt` |
| XE | `xe` | `schedule.evolve` | `evolve` | `xe` |

The old tools `schedule.train` and `schedule.plus` and CLI commands `train` and `plus` have been replaced. Update clients to the names above. Rust callers use `xg::create`, `xh::search`, `xt::search` and `xe::evolve`; the former `engine` and `evolution` modules and `search::train`/`search::plus` entry points are no longer exported.

Previously saved JSON options still accept `trainer`, `plus`, `evolution`, `improve.trainer_share` and `improve.evolution_share` as input aliases. New output and schemas use `xh`, `xt`, `xe`, `improve.xh_share` and `improve.xe_share`. Supplying both names for the same field is rejected. This compatibility applies to saved input, not old tool names. Search reports use `XH`, `XT`, `XE`, or `XHT`/`XHTE` for combined improvement; phase names are `XH`, `XT:1`, `XT:2`, etc., and optionally `XE`. Stored historical reports retain their original labels.
...
| `schedule.hypersearch` / `schedule.repair` | Evolution of staged Q policies and unpinned route/material-mode/conditional choices, with elitism and optional Pareto selection | First active time, evaluation or generation limit |
| `schedule.treesearch` | Persistent prefix tree, UCT branch selection and randomized top-k complete rollouts | First active time or evaluation limit |
| `schedule.improve` | XH/XT portfolio, with an optional XE reserve (default zero after mixed quality ablations) | One shared time/evaluation limit |
...
Repair reconstructs schedules. XT replays prefixes into fresh construction/decoding state; it does not yet maintain incremental decoder snapshots. Neither search proves optimality or infeasibility.
...
```json
{
  "options": {
    ...
    "xt": { "workers": 4, "branching": 4, "depth": 8, "exploration": 1.4 }
  }
}
```

Defaults are 128 evaluations, 5 seconds, population 16, one worker and lexicographic selection. There is no default generation limit. The UI supplies its displayed settings explicitly. `iterations` is a maximum number of complete evaluations, **not generations**. `xh.max_evaluations` overrides it when supplied. Zero evaluations/time disables that limit; at least one applicable limit must remain. XT requires time or evaluations even if an XH generation limit is present.
...
XH evaluates immutable inputs in scoped Rust threads, sorts results back into submission order and selects the next generation after the batch. Fixed seed/evaluation/generation settings reproduce the result across worker counts when time is disabled and customizations are deterministic. XT batches use virtual visits; changing its worker count can change the exploration path. Same seed, worker count and evaluation budget reproduce its path. Time-limited runs can differ with machine load. Memory grows with concurrent full evaluations, so maximum worker count is not necessarily fastest.
...
The [migration audit](https://github.com/qunevo/apex/blob/main/dev/docs/migration-audit.md) consolidates historical v2 decoder comparisons, intentional corrections and unverified migration areas. No unrestricted full-system v2 parity claim is made.
...
The construction-time filter gap identified by the [migration audit](https://github.com/qunevo/apex/blob/main/dev/docs/migration-audit.md) is now addressed by the bounded [planning language and native filter hook](architecture/declarative-scheduling.md). All strategies, XH evaluations, XT branch enumeration, XE candidates and forced prefixes use the same mandatory filtering stage. Detailed semantics deliberately improve on selected legacy defects and do not claim complete customer-specific parity.
