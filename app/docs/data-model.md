...
XG initially favors the route with the lowest estimated work. XH varies route/mode choices with reproducible seeds alongside dispatch strategies. Selection is heuristic, not exhaustive optimization.
...
| `Options` | Staged Q policy, route/main/conditional choices, decision prefix/order, XH, XT, combined-improvement and XE settings |
...
- **XH:** optional conditional-mode genes join staged Qs and workplans. Mutation changes one unprotected conditional mode with probability 0.6 when such alternatives exist; crossover exchanges per-task conditional maps. Main-mode/sequence-dependent choices may be infeasible and then count as failed evaluations. Input and explicitly supplied option commitments are protected.
- **XT:** branches on actually present uncommitted conditional alternatives in the selected prefix, then on subsequent task/main-mode decisions. Conditional branching also works at the configured task-prefix depth boundary. The full evaluation/time limits still apply.
- **Combined improvement:** carries conditional choices from XH/incumbent into XT seed policies. Those choices define that XT branch's policy; another portfolio seed can explore other alternatives. The optional XE also evolves conditional choices under the same hard commitments; see [direct evolution](direct-schedule-evolution.md).
