...
Use `schedule.create` for a quick plan and `schedule.improve` for XH followed by XT and an optional direct schedule GA under one shared budget. An optional current-revision incumbent is independently checked and retained. See the [combined-search contract](architecture/combined-improvement.md) for budget and incumbent semantics.

## XG, XH and repair

XG selects a route per choice, constructs a dependency-compatible dispatch order, chooses eligible execution modes and places phases and conditional graphs using indexed occupancy profiles. It scores and independently validates the complete result before returning it.

XH now evolves per-stage Q weight maps, normalization, unpinned route choices and conditional-resource modes with mutation, crossover and elitism. Selection can use weighted priority levels or Pareto fronts/crowding. XT maintains an alternative-prefix tree with UCT selection, parallel complete rollouts and revisits. Read [the detailed search contract](search-and-parity.md) for limits, defaults, gene operators, proxy mapping and reproducibility. Both return replayable options and search evidence; metrics always come from complete validated schedules.

XG, XH, XT and XE enforce the same hard constraints. XH searches for better candidates; it does not provide a stronger correctness certificate than the independent validator already used by XG. Time budgets are checked between complete evaluations, so one evaluation can exceed the remaining budget.
