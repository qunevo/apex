...
try {
  ...
  const improved = await call('schedule.improve', { scenario_id: imported.scenario_id, schedule_id:baseline.schedule_id, options:{iterations:20,budget_ms:0,improve:{xe_share:0.3},xh:{population_size:4}} });
  assert.equal(improved.search.algorithm,'XHTE');assert.equal(improved.search.phases.at(-1).name,'XE');assert.equal(improved.search.operators.length,10);
  ...
  const xh = await call('schedule.hypersearch', { scenario_id: fork.scenario_id, options: { iterations: 6, budget_ms: 10000 } });
  assert.ok(xh.metrics.urgency_completion > 0);
  results.push({ case: 'agent-defined objective and compiled dispatch signal', passed: true, selected_strategy: xh.selected_strategy, metrics: xh.metrics });
  ...
  for (const name of ['schedule.hypersearch','schedule.treesearch']) {
  ...
  fs.writeFileSync(path.join(root, process.env.APEX_REPORT || 'docs/reports/agent-workflows.json'), JSON.stringify({ sdk: 'official @modelcontextprotocol/sdk', engine: caps.version, results }, null, 2));
