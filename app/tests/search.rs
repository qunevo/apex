use apex::{compile, demo, model::*, queues, rules, search, validate, xg};
use serde_json::json;
...
#[test]
fn xh_honors_generations_evaluations_and_produces_replayable_elite() {
    let p = problem(10);
    let mut o = options();
    o.iterations = 0;
    ...
    assert!(s.score <= xg::create(&p, &o).unwrap().score);
...
fn time_limit_and_unbounded_search_rejection() {
    ...
    o.budget_ms = 0;
    assert!(apex::xh::search(&p, &o).is_err());
    o.xh.generations = Some(1);
    assert!(apex::xt::search(&p, &o).is_err());
}
...
fn overflowing_objective_is_a_diagnostic_not_a_search_panic() {
    ...
    assert!(
        xg::create(&p, &Options::default())
            .unwrap_err()
            .iter()
            .any(|e| e.code == "NUMERIC_OVERFLOW")
    );
    assert!(apex::xh::search(&p, &options()).is_err());
}

#[test]
fn parallel_xh_matches_serial_with_fixed_evaluation_budget() {
    let p = demo::problem(35);
    let a = apex::xh::search(&p, &options()).unwrap();
    let mut o = options();
    o.xh.workers = 4;
    let b = apex::xh::search(&p, &o).unwrap();
...
fn crossover_exchanges_stage_blocks_and_mutation_preserves_protected_gene() {
    ...
    let mut mixed = false;
    for _ in 0..50 {
        let c = apex::xh::crossover(&a, &b, &mut rng);
    ...
    assert!(mixed);
    for _ in 0..100 {
        apex::xh::mutate(&problem(2), &mut a, &Options::default(), &mut rng);
        assert_eq!(
...
fn pareto_archive_keeps_conflicting_goals_and_every_entry_replays() {
    ...
    for entry in archive {
        let candidate = xg::create(&p, &entry.options).unwrap();
...
#[test]
fn xt_backtracks_after_infeasible_baseline_and_revisits_branches() {
    let mut p = problem(2);
    ...
    assert!(validate::validate(&p, &s).valid);
    let again = apex::xt::search(&p, &o).unwrap();
...
fn objective_change_reverses_the_actual_preferred_schedule() {
    ...
    let minimum = apex::xh::search(&p, &options()).unwrap();
    p.objectives[0].maximize = true;
    let maximum = apex::xh::search(&p, &options()).unwrap();
