...
try {
  ...
  assert.equal(evolved.search.algorithm,'XE');assert.equal(evolved.evaluations,24);
  assert.equal((await call('schedule.validate',{schedule_id:evolved.schedule_id})).valid,true);
  const improved=await call('schedule.improve',{scenario_id:imported.scenario_id,schedule_id:baseline.schedule_id,options:{iterations:24,budget_ms:0,improve:{xe_share:0.3},xh:{population_size:4}}});
  assert.equal(improved.search.algorithm,'XHTE');assert.equal(improved.search.phases.at(-1).name,'XE');assert.equal(improved.search.operators.length,10);assert.equal(improved.evaluations,24);
  ...
  const candidate=await call('schedule.hypersearch',{scenario_id:fork.scenario_id,options:{iterations:18,budget_ms:10000}});
  ...
  const elite=await call('schedule.create',{scenario_id:fork.scenario_id,options:xh.replay});assert.deepEqual(elite.score,xh.score);
  const xt=await call('schedule.treesearch',{scenario_id:fork.scenario_id,options:searchOptions});assert.equal(xt.search.algorithm,'XT');assert.equal(xt.search.evaluations,32);assert.ok(xt.search.nodes>1);
  assert.equal((await call('schedule.validate',{schedule_id:xt.schedule_id})).valid,true);
  const freeze=await call('scenario.freeze',{scenario_id:fork.scenario_id,expected_revision:3,schedule_id:xt.schedule_id,until:1000,dimensions:['resource','order']});
  ...
  const optionsSchema=await call('schema.get',{model:'options'});assert.ok(optionsSchema.root.properties.xh);
  results.push({case:'XH generations, mutation, crossover, parallel Pareto selection, replay, XT tree and freeze annotations through official SDK',passed:true,xh:{...xh.search,archive:undefined},xt:{...xt.search,archive:undefined},frozen_tasks:freeze.frozen_tasks});
  fs.writeFileSync(path.join(root,'docs/reports/http-workflows.json'),JSON.stringify({transport:'Streamable HTTP / official MCP SDK',engine:capabilities.version,results},null,2));
