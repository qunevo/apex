...
fn material_routes_urgency_conditionals_and_kpis_are_available_to_agents() {
    ...
    let plan = call(
        &s,
        "schedule.improve",
        json!({"scenario_id":id,"options":{"iterations":24,"budget_ms":0,"xh":{"population_size":4}}}),
    );
    let args =
...
fn improve_tool_retains_current_incumbent_and_rejects_stale_or_foreign_results() {
    ...
    let args = json!({"scenario_id":id,"schedule_id":base["schedule_id"],"options":{"iterations":16,"budget_ms":0,"improve":{"xe_share":0.3},"xh":{"population_size":4}}});
    let improved = call(&s, "schedule.improve", args.clone());
    assert_eq!(improved["search"]["algorithm"], "XHTE");
    assert_eq!(improved["evaluations"], 16);
    let evolved = call(&s, "schedule.evolve", args.clone());
    assert_eq!(evolved["search"]["algorithm"], "XE");
