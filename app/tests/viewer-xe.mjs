...
const improved=await call('schedule.improve',{scenario_id,schedule_id:baseline.schedule_id,options:{iterations:96,budget_ms:0,improve:{xe_share:0.3},xh:{population_size:8,workers:2},xt:{workers:2}}});
assert.equal(improved.search.algorithm,'XHTE');
assert.equal(improved.evaluations,96);
assert.equal(improved.search.phases.at(-1).name,'XE');
...
try{
  await page.goto(url.href);await page.waitForSelector('#xe-operators');
  assert.equal(await page.locator('#xe-operators').evaluate(e=>e.open),false);
  assert.equal(await page.locator('#improve').isVisible(),false);
  assert.match(await page.locator('#model-content').textContent(),/XE/);
  await page.locator('#xe-operators summary').click();
  assert.equal(await page.locator('#xe-operators tr').count(),11);
  assert.match(await page.locator('#xe-operators').textContent(),/apex:job_crossover@2/);
  assert.match(await page.locator('#xe-operators').textContent(),/Skipped/);
  await page.screenshot({path:path.join(reportDir,'xe-viewer.png'),fullPage:true});
  ...
  fs.writeFileSync(path.join(reportDir,'viewer-xe.json'),JSON.stringify(report,null,2));
