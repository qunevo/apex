use apex::{demo, model::*, rules::Customization, validate::validate, xg};
...
#[test]
fn compiled_decorator_is_enforced_by_planner_xh_and_validator() {
    let input = demo::problem(10);
    let (compiled, s) =
        xg::create_customized(&input, &Options::default(), &SyntheticShiftPolicy).unwrap();
    assert!(s.assignments[0].start >= 300);
    assert!(s.metrics.contains_key("urgency_completion"));
    assert!(input.rules.is_empty());
    let xh_result = apex::xh::search(&compiled, &Options::default()).unwrap();
    assert!(xh_result.score <= s.score);
    let mut corrupt = xh_result;
...
fn native_sequence_activities_are_recreated_by_independent_validation() {
    ...
    let (_, s) = xg::create_customized(&p, &Options::default(), &policy).unwrap();
    ...
    let xh_result = apex::xh::search_customized(&p, &Options::default(), &policy).unwrap();
    assert!(apex::validate::validate_customized(&p, &xh_result, &policy).valid);
    let mut corrupt = xh_result;
...
fn native_hard_validation_rejects_both_planning_modes_and_saved_results() {
    ...
    assert!(xg::create_customized(&p, &Options::default(), &policy).is_err());
    assert!(apex::xh::search_customized(&p, &Options::default(), &policy).is_err());
    assert!(!apex::validate::validate_customized(&p, &s, &policy).valid);
}
