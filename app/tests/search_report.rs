...
use apex::{compile, demo, model::*, queues, search, validate, xg};
use serde_json::json;
...
fn measured_search_and_proxy_quality() {
    ...
    for (name, p) in [
        ...
        for run in 0..3 {
            ...
            let xg_result = xg::create(&p, &base).unwrap();
            timings.push(json!({"dataset":name,"method":xg::ID,"workers":1,"run":run,"milliseconds":xg_result.elapsed_ms,"score":xg_result.score,"evaluations":1}));
            for workers in [1, 4] {
                let mut o = base.clone();
                o.xh.workers = workers;
                o.xt.workers = workers;
                for (method, s) in [
                    (apex::xh::ID, apex::xh::search(&p, &o).unwrap()),
                    (apex::xt::ID, apex::xt::search(&p, &o).unwrap()),
                ] {
                    assert!(validate::validate(&p, &s).valid);
                    assert!(s.score <= xg_result.score);
    ...
    let medians: BTreeMap<_, _> = medians
        ...
        .collect();
    let report = json!({"xg":env!("CARGO_PKG_VERSION"),"profile":"release","os":std::env::consts::OS,"hardware_threads":std::thread::available_parallelism().map(|n|n.get()).unwrap_or(1),"timing_contract":"24 complete evaluations, no wall-clock limit, three repeats; includes compilation, construction, decoding and validation; API serialization excluded. XT also reconstructs prefixes.","median_milliseconds":medians,"timings":timings,"proxy_quality_contract":"40 synthetic 12-task instances (20 initial and 20 holdout), first-decision normalized Q cost vs full greedy completion tardiness for each forced first choice. Spearman ties averaged; null means constant signal. This is a diagnostic, not a universal correlation guarantee.","proxy_quality":quality});
    std::fs::write(
        "docs/reports/search-measurements.json",
