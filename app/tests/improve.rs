use apex::{demo, improve, model::*, rules::Customization, validate, xg};
use serde_json::json;
...
fn portfolio_shares_budget_hands_off_distinct_policies_and_preserves_incumbents() {
    ...
    let replay = xg::create(&p, result.replay.as_deref().unwrap()).unwrap();
...
fn campaign_and_fixed_start_hold_through_every_phase_and_parallel_workers() {
    ...
    for candidate in &s.search.as_ref().unwrap().archive {
        let replay = xg::create(&p, &candidate.options).unwrap();
...
fn declarative_goal_and_route_choices_are_replayable() {
    ...
    let replay = xg::create(&p, s.replay.as_deref().unwrap()).unwrap();
...
#[test]
fn soft_global_deadline_does_not_restart_a_fresh_budget_for_xt() {
    let p = demo::problem(3);
    let mut o = options();
    o.budget_ms = 1;
    o.iterations = 0;
...
#[test]
fn xt_recovers_when_the_entire_xh_phase_is_infeasible() {
    ...
    assert!(
        r.phases
            .iter()
            .any(|p| p.name.starts_with("XT:") && p.best_score.is_some())
    );
    assert!(validate::validate(&p, &s).valid);
}
