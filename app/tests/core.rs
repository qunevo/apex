use apex::{compile::compile, demo, model::*, validate::validate, xg};
...
fn run(p: &Problem) -> Schedule {
    xg::create(p, &Options::default()).unwrap_or_else(|e| panic!("{e:#?}"))
}
...
fn c10_terminal_post_and_missing_transition() {
    ...
    let e = xg::create(&p, &Options::default()).unwrap_err();
...
#[test]
fn xh_preserves_valid_incumbent() {
    ...
    .unwrap();
    assert!(xh_result.score <= baseline.score);
    assert_eq!(xh_result.evaluations, 12);
    assert!(validate(&p, &xh_result).valid);
}

#[test]
fn weighted_xh_is_reproducible_and_replayable() {
    ...
    let b = apex::xh::search(&p, &options).unwrap();
    assert_eq!(a.score, b.score);
    assert_eq!(a.strategy, b.strategy);
    assert_eq!(a.seed, b.seed);
    let replay = xg::create(&p, a.replay.as_ref().unwrap()).unwrap();
