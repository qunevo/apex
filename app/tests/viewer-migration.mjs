...
const improved=await call('schedule.improve',{scenario_id,schedule_id:baseline.schedule_id,options:{iterations:48,budget_ms:0,xh:{population_size:4}}});
...
try {
  ...
  await page.screenshot({path:path.join(root,'docs/reports/chain-viewer.png'),fullPage:true});
  ...
  await page.screenshot({path:path.join(root,'docs/reports/kpis.png'),fullPage:true});
  await page.setViewportSize({width:800,height:1000});
  assert.ok(await page.evaluate(()=>document.documentElement.scrollWidth<=window.innerWidth+2));
  assert.deepEqual(errors,[]);
  fs.writeFileSync(path.join(root,'docs/reports/viewer-migration.json'),JSON.stringify({passed:true,url:url.href,baseline_metrics:baseline.metrics,improved_metrics:improved.metrics,baseline_ms:baseline.elapsed_ms,improve_ms:improved.elapsed_ms,material_rows:materials.total,page_errors:errors},null,2));
