...
3. If a new executable rule is needed, add its typed representation to `src/rust/model.rs`, readiness checks to `compile.rs`, evaluation and dispatch semantics to `rules.rs`/`engine.rs`, and final-assignment checks to `validate.rs`.
...
APEX 0.6 adds `evolution_operators` / `evolve` for deterministic mutation or crossover proposals and `conditional_modes` for advance names of sequence-generated alternatives. See [the contract](../../docs/direct-schedule-evolution.md). The synthetic acceptance examples are in `tests/evolution.rs`; this knowledge note does not register another runtime operator. Explicit locks remain protected and independent validation remains authoritative.
