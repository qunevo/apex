...
class WikiTests(unittest.TestCase):
    def setUp(self):
        ...
        self.groups = {"Guides": {"app/docs/README.md": "Home", "app/docs/guide.md": "Guide"}}
        self.put("app/docs/README.md", "# APEX documentation\n\n[Guide](guide.md#setup)\n")
        self.put("app/docs/guide.md", "# Example guide\n\n## Setup\n")
        self.put("app/examples/input.json", "{}\n")
    ...
    def test_page_navigation_sources_and_only_allowlisted_pages(self):
        ...
        self.assertIn(f"/blob/{REVISION}/app/docs/guide.md", pages["Guide.md"])
        self.assertIn("/edit/main/app/docs/guide.md", pages["Guide.md"])

    def test_code_labels_titles_encoded_paths_images_and_reference_links(self):
        self.put("app/examples/with space.json", "{}")
        self.put("app/docs/image.svg", "<svg />")
        self.put("app/docs/README.md", '# Home\n\n[`input`](../examples/input.json "Input")\n[spaced](<../examples/with%20space.json>)\n![image](image.svg)\n[guide][reference]\n\n[reference]: guide.md#setup "Guide"\n[dir](../examples)\n')
        result = self.build()["Home.md"]
        self.assertIn(f'[`input`](https://github.com/example/synthetic/blob/{REVISION}/app/examples/input.json "Input")', result)
        self.assertIn("with%20space.json>", result)
        self.assertIn(f"https://raw.githubusercontent.com/example/synthetic/{REVISION}/app/docs/image.svg", result)
        self.assertIn('[reference]: https://github.com/example/synthetic/wiki/Guide#setup "Guide"', result)
        self.assertIn(f"/tree/{REVISION}/app/examples", result)
    ...
    def test_missing_untracked_and_escaping_links_fail(self):
        self.put("app/docs/local.md", "# Local\n", tracked=False)
        for destination in ["missing.md", "local.md", "../../outside.md", "/etc/passwd", "javascript:alert(1)"]:
            with self.subTest(destination=destination):
                self.put("app/docs/README.md", f"# Home\n[x]({destination})\n")
                with self.assertRaises(ValueError):
                    self.build()

    def test_untracked_source_and_duplicate_slug_fail(self):
        self.put("app/docs/private.md", "# Private\n", tracked=False)
        self.groups["Guides"]["app/docs/private.md"] = "Private"
        self.manifest()
        with self.assertRaisesRegex(ValueError, "tracked Markdown"):
            self.build()
        del self.groups["Guides"]["app/docs/private.md"]
        self.groups["Guides"]["app/docs/guide.md"] = "home"
        self.manifest()
