...
Immediately before merging, verify the expected head SHA and target. Use the normal merge endpoint, preserving merge commits so ancestry between long-lived branches remains clear. Verify the merged SHA and resulting `dev` CI. Do not merge other open branches or delete worktrees as incidental cleanup. Report the PR link and integration result.
