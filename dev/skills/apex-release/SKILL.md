...
Open and merge a normal `main` to `dev` synchronization PR after its required checks pass. Include hotfixes and the version commit; retain newer development work. Attach this PR too. Verify that the released `main` commit is an ancestor of `dev` and compare their tracked content; expect equality unless newer feature work entered `dev`, in which case report that difference. Complete the task's remote and local release/hotfix branch cleanup using [branch lifecycle](../../docs/developer-workflow.md#branch-lifecycle), preserving both permanent branches and active checkouts. Release authorization includes this cleanup; do not leave the temporary branch merely because its PR is merged. Report the product version, release and both PR links, publication state, branch cleanup and remaining blockers.
